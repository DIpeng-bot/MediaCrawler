...
class DOUYINClient(AbstractApiClient):
    ...
    async def get_video_by_id(self, aweme_id: str) -> Any:
        ...
        headers = copy.copy(self.headers)
        del headers["Origin"]
        utils.logger.debug(f"Fetching video details for aweme_id: {aweme_id}")
        res = await self.get("/aweme/v1/web/aweme/detail/", params, headers)
        utils.logger.debug(f"Got response for video details: {res.keys()}")
    ...
    async def download_video(self, video_url: str, save_path: str) -> None:
        """Download video from url"""
        max_retries = config.VIDEO_DOWNLOAD_RETRIES
        retry_count = 0
        ...
        try:
            ...
            while retry_count < max_retries:
                try:
                    utils.logger.debug(f"Starting video download from: {video_url}")
                    headers = {
                    ...
                    if 'aweme.snssdk.com' in video_url:
                        video_url = video_url.replace('aweme.snssdk.com', 'api.amemv.com')
                    
                    utils.logger.debug(f"Processed video URL: {video_url}")
                    ...
                    with requests.Session() as session:
                        ...
                        utils.logger.debug(f"Download response status: {response.status_code}")
                        
                        # Handle both 200 (OK) and 206 (Partial Content) responses
                        if response.status_code in [200, 206]:
                            total_size = int(response.headers.get('content-length', 0))
                            block_size = config.VIDEO_CHUNK_SIZE
                            progress = 0
                                    ...
                                    for chunk in response.iter_content(chunk_size=block_size):
                                        if chunk:
                                            ...
                                            if total_size > 0:
                                                ...
                                                utils.logger.debug(f"Download progress: {percentage:.2f}%")
                                
                                # Verify the downloaded file
                                if os.path.exists(temp_path) and os.path.getsize(temp_path) > 0:
                                    # Only move the file if it's complete
                                    if os.path.exists(save_path):
                                        os.remove(save_path)
                                    os.rename(temp_path, save_path)
                                    utils.logger.info(f"Video downloaded successfully: {save_path}")
                                    return
                                else:
                                    utils.logger.error("Downloaded file is empty")
                                    retry_count += 1
                            except Exception as e:
                                utils.logger.error(f"Error writing file: {str(e)}")
                                if os.path.exists(temp_path):
                                    os.remove(temp_path)
                                retry_count += 1
                        else:
                            utils.logger.error(f"Failed to download video, status code: {response.status_code}")
                            retry_count += 1
                        
                        if retry_count < max_retries:
                            utils.logger.info(f"Retrying download ({retry_count}/{max_retries})...")
                            await asyncio.sleep(config.VIDEO_DOWNLOAD_RETRY_DELAY)
                            
                except Exception as e:
                    utils.logger.error(f"Error downloading video: {str(e)}")
            ...
            utils.logger.error(f"Failed to download video after {max_retries} attempts")
            
        except Exception as e:
            utils.logger.error(f"Fatal error in download_video: {str(e)}")
            utils.logger.exception("Detailed error:")
        finally:
            ...
            if os.path.exists(temp_path):
                try:
                    ...
                    utils.logger.error(f"Error removing temporary file: {str(e)}")
